...
AS
BEGIN
    SELECT nameProduct, priceProduct, imageProduct, typeProduct FROM Product
END
go
...
AS
BEGIN
	SELECT id, price, amount, paymethod, status, paydate FROM OrderHistory WHERE idUser = @idUser
END
go
...
AS
BEGIN
	INSERT INTO Product (nameProduct, priceProduct, imageProduct, typeProduct) VALUES ( @nameProduct , @priceProduct , @imageProduct , @typeProduct )
END
go
